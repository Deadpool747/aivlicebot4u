...
- For post-call transcription: `CALL_TRANSCRIPTS_DB_PATH`, `FASTER_STT_ENABLED`, `FASTER_STT_MODEL`, `FASTER_STT_DEVICE`, `FASTER_STT_COMPUTE_TYPE`, `POST_CALL_SUMMARY_ENABLED`, `POST_CALL_WHATSAPP_NOTIFICATION_ENABLED`, `CALL_TRANSCRIPTION_WHATSAPP_NUMBER`
...
The post-call transcription flow now uses Faster-Whisper for saved call recordings, so you can tune the local model via `FASTER_STT_MODEL` and `FASTER_STT_DEVICE` without involving Google Cloud Speech-to-Text. This keeps the live call path and saved-recording transcription path separate.
