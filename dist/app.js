...
function finish(){ending=false;attempt++;connected=false;updateScriptButton();socket?.close();socket=null;capture?.disconnect();source?.disconnect();silentGain?.disconnect();stream?.getTracks().forEach(t=>t.stop());stream=null;stopAudio();context?.close();context=null;capture=null;muted=false;$('mute').textContent='Mute microphone';$('mute').setAttribute('aria-pressed','false');$('nameField').hidden=false;$('connect').hidden=false;$('connect').disabled=!$('customerName').value.trim();$('customerName').disabled=false;$('outboundNumber').disabled=false;$('customerEmail').disabled=false;$('callMode').disabled=false;$('connect').textContent='Start conversation';$('session').hidden=true;$('send').disabled=true;$('status').textContent='Conversation ended.';userLine=null;agentLine=null}
function transcript(role,text){if(!text)return;let el=role==='user'?userLine:agentLine;if(!el){el=document.createElement('div');el.className='message '+role;const label=document.createElement('strong');label.textContent=role==='user'?'You':'BOT4U';const body=document.createElement('span');el.append(label,body);$('messages').append(el);if(role==='user')userLine=el;else agentLine=el}el.lastChild.textContent+=text;el.scrollIntoView({block:'nearest'})}
...
async function connect(){if(!scriptLoaded||savingScript){error('Wait for the script to load or save.');return}if(!savedScript.trim()){error('Add and save a script for this call mode first.');$('script').focus();return}if($('script').value!==savedScript){error('Save your script changes before starting the conversation.');$('saveScript').focus();return}const phone=$('outboundNumber').value.trim();if(mode==='outbound'&&phone&&!/^\+[1-9]\d{6,14}$/.test(phone.replace(/[ ()-]/g,''))){error('Enter a phone number with country code, for example +91 98765 43210.');$('outboundNumber').focus();return}const email=$('customerEmail').value.trim();if(email&&!$('customerEmail').checkValidity()){error('Enter a valid email address.');$('customerEmail').focus();return}const name=$('customerName').value.trim();if(!/^[\p{L}\p{M} .’'-]{1,80}$/u.test(name)){error('Enter a client name using letters, spaces, apostrophes or hyphens.');$('customerName').focus();return}const id=++attempt;$('connect').disabled=true;$('customerName').disabled=true;$('outboundNumber').disabled=true;$('customerEmail').disabled=true;$('callMode').disabled=true;error('');$('status').textContent='Connecting…';try{
...
$('customerName').oninput=()=>{$('connect').disabled=!$('customerName').value.trim()};
...
$('chat').onsubmit=e=>{e.preventDefault();const text=$('text').value.trim();if(!connected||ending||!text)return;stopAudio();userLine=null;agentLine=null;transcript('user',text);socket.send(JSON.stringify({text}));$('text').value=''};
