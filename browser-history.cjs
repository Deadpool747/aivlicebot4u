function trackBrowserCall(history,owner,{name,mode},now=Date.now){
 ...
 let connectedAt=null,finished=false;
 ...
 return {
  ...
  end(){if(finished)return;finished=true;record(connectedAt===null?'Not answered':'Answered',connectedAt===null?'Website demo could not connect.':'Website demo ended.',connectedAt===null?null:Math.max(0,Math.round((now()-connectedAt)/1000)))}
 };
}
