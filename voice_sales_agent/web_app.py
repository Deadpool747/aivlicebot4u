...
class BrowserAudioBridge:
    ...
    def should_drop_input_while_playing(self) -> bool:
        # Browser playback can leak into browser mic; gate input while playback is active.
        return True
    ...
    async def stop(self, session_key: str = "dashboard") -> None:
        ...
        if task is not None:
            try:
                ...
                logger.warning(
                    "Session finalize exceeded 15s; keeping task alive to complete persistence session_key=%s",
                    session_key,
                )
            except Exception:
...
class TelephonyController:
    ...
    async def activate_consent_call(self, pending_id: str, pending_call: PendingCall) -> dict[str, Any]:
        ...
        if target_provider not in {"twilio", "exotel", "airtel_iq", "meta_whatsapp", "tata"}:
...
def create_app() -> FastAPI:
    ...
    async def internal_start_phone_call(request: StartPhoneCallRequest, http_request: Request) -> dict[str, str]:
        ...
        if provider_override in {"twilio", "tata", "exotel", "airtel_iq", "meta_whatsapp"}:
    ...
    async def tata_inbound_voice_webhook(request: Request) -> dict[str, Any]:
        ...
        target_client_id, target_project_id = _resolve_inbound_tata_target(to_number=to_number)
        try:
            pending_id, _ = await telephony.create_inbound_call(
                provider="tata",
                client_id=target_client_id,
                ...
                project_id=target_project_id,
                provider_call_sid=call_sid or None,
                lead_source="tata_inbound_stream",
                context_metadata={
                    ...
                    "cdr_duration_seconds": duration_seconds or None,
                    "status_callback_payload": json.dumps(payload, ensure_ascii=True),
                    "auto_created_from_tata_inbound_webhook": True,
                },
            )
            logger.info(
                "Processed Tata inbound webhook by creating pending_id=%s payload=%s",
                pending_id,
                payload,
            )
            asyncio.create_task(
            ...
            logger.info("Processed Tata inbound webhook without pending match payload=%s", payload)
        return {"status": "ok"}
    ...
    async def _run_tata_media_session(websocket: WebSocket, pending_id: str | None) -> None:
        ...
        try:
            while True:
                    ...
                    if pending_call is None and not auto_created_inbound_pending and event in {"connected", "start", "media"}:
                        ...
                        auto_pending_id, auto_pending_call = await telephony.create_inbound_call(
                            ...
                            context_metadata={
                                "call_direction": direction,
                                "called_via_number": to_number or None,
                                "auto_created_from_tata_ws": True,
                            },
                        )
                ...
                if not session_started and event in {"connected", "start", "media"}:
                    target_client_id, target_project_id = _resolve_inbound_tata_target(
                        to_number=(
                    ...
                    logger.info(
                        "Tata session start selection: pending_id=%s provider_call_sid=%s pending_client_id=%s selected_client_id=%s selected_project_id=%s",
                        active_session_key,
            ...
            logger.info("Tata media websocket disconnected pending_id=%s", pending_id or "<fallback>")
        except RuntimeError:
            logger.exception("Tata media session failed pending_id=%s", pending_id or "<fallback>")
            await bridge.close()
        finally:
