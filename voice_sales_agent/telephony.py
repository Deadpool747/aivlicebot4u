...
TATA_DEFAULT_HEADERS = {"Content-Type": "application/json"}
TATA_DEFAULT_REQUEST_TEMPLATE = {
    "agent_number": "{agent_number}",
    "destination_number": "{destination_number}",
    "caller_id": "{caller_id}",
    "async": 1,
}
_tata_packet_bytes_env = str(os.getenv("TATA_PACKET_BYTES", "160")).strip() or "160"
try:
    _tata_packet_bytes = int(_tata_packet_bytes_env)
except ValueError:
    _tata_packet_bytes = 160
# G.711 u-law @ 8kHz is 1 byte/sample; 160 bytes == 20 ms.
# Keep packet size frame-aligned by requiring multiples of 160.
if _tata_packet_bytes < 160:
    _tata_packet_bytes = 160
if _tata_packet_bytes % 160 != 0:
    _tata_packet_bytes = ((_tata_packet_bytes // 160) + 1) * 160
TATA_MULAW_FRAME_BYTES = _tata_packet_bytes
TATA_FRAME_MS = int((TATA_MULAW_FRAME_BYTES / 8000.0) * 1000)
TATA_PCM16_FRAME_BYTES = TATA_MULAW_FRAME_BYTES * PCM16_WIDTH
TATA_MAX_RECOMMENDED_BATCH_BYTES = 1_600
...
class TataMediaBridge(TwilioMediaBridge):
    """Tata bidirectional media payloads match Twilio-style mulaw/8000 websocket events."""

    def __init__(self, websocket: WebSocket) -> None:
        super().__init__(websocket)
        self._outbound_queue: asyncio.Queue[bytes] = asyncio.Queue(maxsize=48)
        self._outbound_worker_task: asyncio.Task[None] = asyncio.create_task(self._outbound_worker())
        self._outbound_pcm8k_buffer = bytearray()
        self._next_send_at = 0.0
        self._inbound_alignment_warnings = 0
        self._outbound_alignment_warnings = 0
        self._capture_enabled = str(os.getenv("TATA_DEBUG_CAPTURE", "")).strip().lower() in {"1", "true", "yes", "on"}
        self._capture_root = Path(os.getenv("TATA_DEBUG_CAPTURE_DIR", "/tmp/tata_debug_capture"))
        self._capture_session_id = f"{int(time.time())}_{id(self)}"
        self._capture_inbound_path: Path | None = None
        self._capture_outbound_path: Path | None = None

    def _capture_slug(self) -> str:
        stream = str(self.stream_sid or "").strip()
        call = str(self.call_sid or "").strip()
        base = call or stream or self._capture_session_id
        return "".join(ch if ch.isalnum() or ch in {"_", "-"} else "_" for ch in base)[:96] or self._capture_session_id

    def _ensure_capture_paths(self) -> None:
        if not self._capture_enabled:
            return
        if self._capture_inbound_path is not None and self._capture_outbound_path is not None:
            return
        self._capture_root.mkdir(parents=True, exist_ok=True)
        slug = self._capture_slug()
        self._capture_inbound_path = self._capture_root / f"tata_inbound_{slug}.ulaw"
        self._capture_outbound_path = self._capture_root / f"tata_outbound_{slug}.ulaw"
        logger.info(
            "Tata debug capture enabled: inbound=%s outbound=%s",
            str(self._capture_inbound_path),
            str(self._capture_outbound_path),
        )

    def _capture_write(self, *, outbound: bool, payload: bytes) -> None:
        if not self._capture_enabled or not payload:
            return
        self._ensure_capture_paths()
        path = self._capture_outbound_path if outbound else self._capture_inbound_path
        if path is None:
            return
    ...
    async def handle_ws_message(self, message: dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.warning("Tata media payload could not be base64-decoded.")
            return
        if len(decoded) % 160 != 0:
            self._inbound_alignment_warnings += 1
            if self._inbound_alignment_warnings <= 5:
                logger.warning(
                    "Tata inbound media frame misalignment detected: bytes=%s not_multiple_of=%s",
                    len(decoded),
                    160,
                )
        self._capture_write(outbound=False, payload=decoded)
        await self._incoming_audio.put(twilio_mulaw_to_pcm16k(decoded))
    ...
    async def play(self, audio_bytes: bytes) -> None:
        ...
        self._playback_idle.clear()
        try:
            self._outbound_queue.put_nowait(audio_bytes)
        except asyncio.QueueFull:
            # Backpressure instead of dropping audio so speech pace stays natural.
            await self._outbound_queue.put(audio_bytes)
    ...
    async def flush_playback(self) -> None:
        ...
        self._outbound_pcm8k_buffer.clear()
        self._playback_active = False
        self._playback_idle.set()
    ...
    async def _outbound_worker(self) -> None:
        try:
            while True:
                chunk_24k = await self._outbound_queue.get()
                try:
                    await self._send_chunk(chunk_24k)
                    if self._outbound_queue.empty() and self._outbound_pcm8k_buffer:
                        # Finalize trailing partial frame with telephony silence.
                        self._flush_partial_frame_with_silence()
                        frame = self._consume_next_pcm_frame()
                        if frame is not None:
                            encoded = pcm16_to_mulaw_bytes(frame)
                            await self._send_mulaw_frame(encoded)
                finally:
                    self._outbound_queue.task_done()
                    if self._outbound_queue.empty() and not self._outbound_pcm8k_buffer:
                        self._playback_active = False
                        self._playback_idle.set()
        except asyncio.CancelledError:
            return

    async def _send_chunk(self, chunk_24k: bytes) -> None:
        if self._closed or self.stream_sid is None or not chunk_24k:
            return

        pcm_8k = _resample_linear(chunk_24k, 24_000, TWILIO_SAMPLE_RATE)
        if not pcm_8k:
            return
        self._outbound_pcm8k_buffer.extend(pcm_8k)
    ...
    def _consume_next_pcm_frame(self) -> bytes | None:
        if len(self._outbound_pcm8k_buffer) < TATA_PCM16_FRAME_BYTES:
            return None
        frame = bytes(self._outbound_pcm8k_buffer[:TATA_PCM16_FRAME_BYTES])
        del self._outbound_pcm8k_buffer[:TATA_PCM16_FRAME_BYTES]
        return frame

    def _flush_partial_frame_with_silence(self) -> None:
        remainder = len(self._outbound_pcm8k_buffer) % TATA_PCM16_FRAME_BYTES
        if remainder == 0:
            return
        # PCM zeroes convert to μ-law 0xFF silence.
        self._outbound_pcm8k_buffer.extend(b"\x00" * (TATA_PCM16_FRAME_BYTES - remainder))

    async def _send_mulaw_frame(self, encoded: bytes) -> None:
        if self._closed or self.stream_sid is None or not encoded:
            return
        if len(encoded) != TATA_MULAW_FRAME_BYTES:
            self._outbound_alignment_warnings += 1
            if self._outbound_alignment_warnings <= 5:
                logger.warning(
                    "Tata outbound media frame misalignment detected: bytes=%s expected=%s",
                    len(encoded),
                    TATA_MULAW_FRAME_BYTES,
                )
            if len(encoded) < TATA_MULAW_FRAME_BYTES:
                encoded = encoded + (b"\xFF" * (TATA_MULAW_FRAME_BYTES - len(encoded)))
            else:
                encoded = encoded[:TATA_MULAW_FRAME_BYTES]

        if len(encoded) > TATA_MAX_RECOMMENDED_BATCH_BYTES:
            logger.warning(
                "Tata outbound media batch is larger than recommended size: bytes=%s recommended_max=%s",
                len(encoded),
                TATA_MAX_RECOMMENDED_BATCH_BYTES,
            )
        ...
        self._next_send_at = max(self._next_send_at, time.monotonic()) + (TATA_FRAME_MS / 1000.0)

        self._capture_write(outbound=True, payload=encoded)
...
class TataCallClient:
    """Create outbound calls through Tata Click-to-Call API."""
    ...
    def ensure_configured(self) -> None:
        missing = [
            ...
            for name, value in (
                ("PUBLIC_BASE_URL", self.settings.public_base_url),
                ("TATA_CLICK2CALL_API_URL", self.settings.tata_click2call_api_url),
                ("TATA_API_TOKEN", self.settings.tata_api_token or self.settings.tata_api_key),
            )
            if not value
        ]
        if missing:
    ...
    def _build_headers(self) -> dict[str, str]:
        ...
        default_auth = (self.settings.tata_api_token or self.settings.tata_api_key or "").strip()
        if default_auth:
            headers["Authorization"] = (
                default_auth
                if default_auth.lower().startswith("bearer ")
                else f"Bearer {default_auth}"
            )
        if self.settings.tata_headers_json:
            try:
                custom_headers = json.loads(self.settings.tata_headers_json)
            except json.JSONDecodeError as exc:
                raise RuntimeError("TATA_HEADERS_JSON must be valid JSON.") from exc
            if not isinstance(custom_headers, dict):
                raise RuntimeError("TATA_HEADERS_JSON must be a JSON object.")
            header_replacements = {
                "api_key": self.settings.tata_api_key or "",
                "api_token": self.settings.tata_api_token or "",
            }
            ...
            for key, value in custom_headers.items():
                ...
                for name, replacement in header_replacements.items():
                    value_text = value_text.replace(f"{{{name}}}", replacement)
                rendered_headers[str(key)] = value_text
    ...
    def _post_json_sync(self, url: str, body: dict[str, Any], headers: dict[str, str]) -> dict[str, Any]:
        ...
        try:
            ...
            body_text = exc.read().decode("utf-8", errors="replace").strip()
            raise RuntimeError(f"Tata Click-to-Call request failed ({exc.code}): {body_text}") from exc
        except urllib.error.URLError as exc:
            raise RuntimeError(f"Could not connect to Tata Click-to-Call API: {exc.reason}") from exc
    ...
    def _create_call_sync(
        ...
        request_template = TATA_DEFAULT_REQUEST_TEMPLATE
        if self.settings.tata_request_template_json:
            try:
                request_template = json.loads(self.settings.tata_request_template_json)
            except json.JSONDecodeError as exc:
                raise RuntimeError("TATA_REQUEST_TEMPLATE_JSON must be valid JSON.") from exc

        resolved_agent_number = (agent_number or self.settings.tata_agent_number or self.settings.tata_caller_id or "").strip()
        resolved_caller_id = (caller_id or self.settings.tata_caller_id or "").strip()
        ...
        replacements = {
            ...
            "async": "1" if self.settings.tata_async else "0",
            "ws_url": ws_url,
            "status_callback_url": status_callback_url or "",
            "events_callback_url": events_callback_url or status_callback_url or "",
            "cdr_callback_url": cdr_callback_url or status_callback_url or "",
            "public_base_url": self.settings.public_base_url or "",
        }
        body = _replace_template_placeholders(request_template, replacements)
        if isinstance(body, dict):
            ...
            if "destination_number" not in body:
                body["destination_number"] = to_number
            if "agent_number" not in body and resolved_agent_number:
                body["agent_number"] = resolved_agent_number
            body["async"] = 1 if str(body.get("async", "1")).strip().lower() in {"1", "true", "yes", "on"} else 0

        parsed = self._post_json_sync(
            self.settings.tata_click2call_api_url or "",
            body if isinstance(body, dict) else {"payload": body},
            self._build_headers(),
        )
        return _normalize_provider_response(parsed)
