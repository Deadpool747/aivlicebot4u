...
function createPhoneCaller({directory,token,readScript,fetcher=fetch,history}){
 ...
 return async function call(owner,data){
  ...
  try{
   ...
   if(body.message!=='call_queued'||typeof body.request!=='string')return fail(502,'Piopiy returned an unexpected response. Check call history before retrying.',true);
   return {status:202,body:{historyId,requestId:body.request,message:'Piopiy queued the call; ringing is not confirmed. Request ID: '+body.request}};
  }catch{return fail(502,'Call status could not be confirmed. Check Piopiy call history before trying again.',true);}
