...
const sessionPrompt=selectedScript.replaceAll('{{customer_name}}',()=>name).replaceAll('{{customer_email}}',()=>email||'not provided')+'\n\n'+voiceDefaults+'\nCall control: After speaking your complete final goodbye, call end_conversation so the app disconnects after playback. Do not call it while awaiting a customer response.\nFollow-ups: If the customer asks for a callback, clarify the exact future calendar date, local time, timezone, customer name and Indian mobile number. Repeat those details and get explicit confirmation before calling schedule_follow_up. Never guess an ambiguous date or time. Current server time is '+new Date().toISOString()+'.';
let ready=false;
...
upstream.on('open',()=>upstream.send(JSON.stringify({setup:{model:`models/${model.replace(/^models\//,'')}`,generationConfig:{responseModalities:['AUDIO'],thinkingConfig:{thinkingLevel:'minimal'},speechConfig:{voiceConfig:{prebuiltVoiceConfig:{voiceName:'Sulafat'}}}},systemInstruction:{parts:[{text:sessionPrompt}]},tools:[{functionDeclarations:[{name:"send_booking_link",description:"Email the discovery-call link only after the customer confirms the exact email address and agrees to receive it. Report only the returned sending status.",parameters:{type:"OBJECT",properties:{email:{type:"STRING"},confirmed:{type:"BOOLEAN"}},required:["email","confirmed"]}},{name:"schedule_follow_up",description:"Schedule a future Airtel callback only after the customer explicitly confirms every detail.",parameters:{type:"OBJECT",properties:{customerName:{type:"STRING"},phoneNumber:{type:"STRING",description:"Indian mobile number in +91 format"},date:{type:"STRING",description:"Confirmed local date as YYYY-MM-DD"},time:{type:"STRING",description:"Confirmed local time as HH:MM in 24-hour format"},timezone:{type:"STRING",description:"IANA timezone, usually Asia/Kolkata"},reason:{type:"STRING"},notes:{type:"STRING"},confirmed:{type:"BOOLEAN"}},required:["customerName","phoneNumber","date","time","timezone","reason","confirmed"]}},{name:"end_conversation",description:"End the call after your spoken closing statement has been delivered. Call only after saying goodbye, never while asking a question or waiting for a reply."}]}],inputAudioTranscription:{},outputAudioTranscription:{},realtimeInputConfig:{automaticActivityDetection:{disabled:false,startOfSpeechSensitivity:'START_SENSITIVITY_LOW',endOfSpeechSensitivity:'END_SENSITIVITY_LOW',prefixPaddingMs:120,silenceDurationMs:650},activityHandling:'START_OF_ACTIVITY_INTERRUPTS'}}})));
upstream.on('message',async raw=>{let data;try{data=JSON.parse(raw)}catch{return}if(data.setupComplete){browserCall.connected();ready=true;clearTimeout(timeout);send({ready:true});upstream.send(JSON.stringify({realtimeInput:{text:'The customer has connected. Greet them now with your short opening question.'}}))}if(data.serverContent)send({serverContent:data.serverContent});for(const call of data.toolCall?.functionCalls||[]){let result;if(call.name==='send_booking_link'){result=await mailer(call.args||{});send({emailResult:result})}else if(call.name==='schedule_follow_up'){try{if(call.args?.confirmed!==true)throw Error('Ask the customer to confirm the exact callback details first.');requireAirtel(owner);result={status:'scheduled',followUp:followUps.create(owner,call.args||{},{source:'voice-agent-browser'})}}catch(e){result={status:'error',error:e.message}}}else if(call.name==='end_conversation'){ready=false;send({endAfterPlayback:true});result={status:'ending_after_playback'}}if(result&&upstream.readyState===WebSocket.OPEN)upstream.send(JSON.stringify({toolResponse:{functionResponses:[{id:call.id,name:call.name,response:result}]}}))}if(data.goAway)send({notice:'This session is ending soon. Reconnect to continue.'});if(data.error){send({error:'Gemini Live could not start. Please reconnect.'});client.close()}});
